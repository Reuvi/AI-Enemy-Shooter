...
function NPCGuard:attack(targetPlayer)
	...
	if targetCharacter and targetCharacter:FindFirstChild("HumanoidRootPart") then
		...
		if self.barrel:isA("Part") then
			projectile.Position = self.barrel.CFrame.Position
		else
		...
		signal = projectile.Touched:Connect(function(hit)
			...
			if hit.Parent:isA("Model") and hit.Parent ~= owner and hit.Parent.Name ~= "M4" then
...
function NPCGuard:Respawn()
	
	self.players = self.zone:getPlayers()
...
function NPCGuard:Start()
	...
	self.humanoid.HealthChanged:Connect(function()
		...
		if self.humanoid.Health <= 0 then
			...
			self.locked = true
			local parent = self.Instance.Parent.Name
			local Clone = script:WaitForChild(self.Instance.Name):Clone()
			self.Instance:Remove()
			task.wait(math.random(3,5))
			Clone.Parent = workspace.NPC.enemyAI:FindFirstChild(parent)
			Clone:SetAttribute("cloned", "true")
			Clone:AddTag("GuardNPC")
	...
	if self.Instance:GetAttribute("cloned") == "true" then
		self:Respawn()
	end
end
